...
<script>
	...
	// URL file GeoJSON
	var geojsonUrl2 = '<?= base_url('/batas_desa_pangandaran.geojson'); ?>';
		...
		.then(data => {
			...
			L.geoJSON(data, {
				...
				onEachFeature: function(feature, layer) {
					...
					layer.on('mouseout', function(e) {
						geojsonLayer2.resetStyle(e.Kd_Inf); // isi dengan nama variabel dari layer
						info.update();
					});
				}
			}).addTo(geojsonLayer2);
	...
	var searchlayer = L.featureGroup([geojsonLayer, geojsonLayer2]);
	...
	L.control.search({
			...
			propertyName: 'Nm_Ruas', // nama field yang akan dijadikan acuan di dalam tool pencarian
