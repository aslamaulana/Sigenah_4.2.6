...
<?php if ($_SESSION['perubahan'] == 'Perubahan') { ?>
	<div class="card-body">
		<form action="<?= base_url('/user/renstra/opd_kegiatan_sub/simpanExcel') ?>" method="POST" enctype="multipart/form-data">
			<?= csrf_field() ?>
			<table class="table table-bordered">
				<tr>
					<td colspan="3">Tambah Data Sub Kegiatan (import dari renstra murni)</td>
				</tr>
				<tr>
					...
					<td>
						<div class="input-group">
							<a href="<?= base_url('/user/renstra/opd_kegiatan_sub/export_perubahan'); ?>">
		...
		<form action="<?= base_url('/user/renstra/opd_kegiatan_sub/simpanExcel') ?>" method="POST" enctype="multipart/form-data">
			...
			<table class="table table-bordered">
				...
				<tr>
					...
					<td>
						<div class="input-group">
							<a href="<?= base_url('/user/renstra/opd_kegiatan_sub/export'); ?>">
								<li class="btn btn-block btn-success btn-sm" active><i class="nav-icon fa fa-file-excel"></i> Format</li>
							</a>
						</div>
					</td>
...
<div class="card-body">
	<form action="<?= base_url('/user/renstra/opd_kegiatan_sub/simpanExcel_edit') ?>" method="POST" enctype="multipart/form-data">
		<?= csrf_field() ?>
		<table class="table table-bordered">
			<tr>
				<td colspan="3">Ubah Data Sub Kegiatan</td>
			</tr>
			<tr>
				<td style="width:120px;">Tahap 1 -> </td>
				<td style="width:220px;">Download Format Excel</td>
				<td>
					<div class="input-group">
						<a href="<?= base_url('/user/renstra/opd_kegiatan_sub/export_edit'); ?>">
							<li class="btn btn-block btn-success btn-sm" active><i class="nav-icon fa fa-file-excel"></i> Format</li>
						</a>
					</div>
				</td>
...
<div class="card-body">
	<table class="table table-bordered">
		<tr>
			<td colspan="3">Hapus Seluruh Data Sub Kegiatan</td>
		</tr>
		<tr>
			<td colspan="3" class="text-center">
				<a class="btn btn-danger btn-circle btn-md" onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){location.href='<?= base_url() . '/user/renstra/opd_kegiatan_sub/hapusAll'; ?>'}" href="#">
