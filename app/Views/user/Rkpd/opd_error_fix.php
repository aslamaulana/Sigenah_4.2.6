...
<div class="card-body">
	<form action="<?= base_url('/user/rkpd/opd_error_fix/simpanExcel_edit') ?>" method="POST" enctype="multipart/form-data">
		<?= csrf_field() ?>
		<table class="table table-bordered">
			<tr>
				<td colspan="3">Renja Sub Kegiatan<br>
					"Dalam proses pengisian renja sub kegiatan ke aplikasi, ada kemungkinan anda akan mengalami error data (Whoops!), error data ini kemungkinan besar di sebabkan kesalahan pengisian data pagu di renja Sub Kegiatan.
					Data Pagu renja Sub Kegiatan harus berupa angka (0-9) dan harus di perhatikan juga kemungkinan terbawanya (space) dan simbol lain yang dapat mengakibatkan error"<br><br>
			...
			<tr>
				...
				<td>
					<div class="input-group">
						<a href="<?= base_url('/user/rkpd/opd_error_fix/export_edit'); ?>">
			...
			<tr>
				...
				<td>
					<?php if (menu('renja')->kunci == 'tidak') { ?>
						<a class="btn btn-danger btn-circle btn-md" onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){location.href='<?= base_url() . '/user/rkpd/opd_error_fix/hapusAll'; ?>'}" href="#">
